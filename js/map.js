            ...
            for (var state in mapConfig.shapes) {
			   ...
			   (function (st, state){					    
					...
					function clickMap(){
						...
						// 绘制热点省份相关数据						
						(function (xx, yy ,state){							
							
							if(oldFrame){							

								// 删除旧热点数据
								oldFrame.remove();	
								for(var i = 0; i< oldPointText.length ; i++){
									oldPointText[i].remove();
								}
							}
							...
							var labelTxt = {font: '10px Helvetica, Arial', fill: "#000"};		
							var label = r.set();
							var point1 = r.text(xx, (side==="bottom"?yy+35:yy-95),    mapConfig['names'][state]).attr(labelTxt);
							var point2 = r.text(xx, (side==="bottom"?yy+50:yy-80), "一定缺口").attr(labelTxt);
							var point3 = r.text(xx, (side==="bottom"?yy+65:yy-65), "平衡值：-1000").attr(labelTxt);
							var point4 = r.text(xx, (side==="bottom"?yy+80:yy-50), "负荷：20584").attr(labelTxt);
							var point5 = r.text(xx, (side==="bottom"?yy+95:yy-35), "工作容量：32123").attr(labelTxt);
							
							// 将热点数据存入数组中， 点击其他省份热点时，删除旧热点数据
							var pointText = new Array(5);
							pointText[0] = point1 ;
							pointText[1] = point2 ;
							pointText[2] = point3 ;
							pointText[3] = point4 ;
							...
							var frame = r.popup(xx, (side==="bottom"?yy+10:yy-10), label, side).attr({fill: "#ffe", stroke: "#666", "stroke-width": 1.5}).hide();
				
							oldFrame = frame ;	
							oldPointText = pointText ;
